...
public class HierarchicalClustering {
    ...
    public static void main(String[] args) throws Exception{
        ...
        hieCluster(conceptWordSet,afterClusterSingleVSMMap, vsmSingleMap, CLUSTER_THRESHOLD);
    }
    ...
    private static void hieCluster(Set<String> conceptWordSet, Map<String, Double> afterClusterSingleVSMMap, Map<String, Double> vsmSingleMap, double simThreshold) throws Exception{
        ...
        double maxSim = 0.0;// 最大相似度
        String maxSimCombineWord = "";// 拥有最大相似度的两个簇
        String maxSimCombineWordOther = "";// 拥有最大相似度的两个簇
     ...
     * @return
     */
    private static Set<String> findParentConcept(Map<String, Double> vsmMap, Set<String> conceptWordSet) throws Exception{
        Set<String> resultSet = new HashSet<String>();
        for (String result : conceptWordSet) {
            if (result.indexOf("|") != -1) {
                String maxSimCombineWord= "";
                double simMax = 0.0;
                String resultArray[] = result.split("\\|");
                for (String resultWord : resultArray) {
                    double simSum = 0.0d;// 全局相似度总和

                    for (String resultOtherWord : resultArray) {
                        if (!resultWord.equals(resultOtherWord)) {
                            String vmsName = resultWord + "|" + resultOtherWord;
                            String vmsReverseName = resultOtherWord + "|" + resultWord;
                            double sim = 0;
                            if (vsmMap.containsKey(vmsName)) {
                                sim = vsmMap.get(vmsName);
                            } else if (vsmMap.containsKey(vmsReverseName)) {
                                sim = vsmMap.get(vmsReverseName);
                            } else {
                                System.out.println("vsmMap不存在key" + vmsName);
                            }
                            simSum += sim;
                        }
                    }

                    double simAvg = simSum / resultArray.length;
                    if (simAvg != 1 && simAvg > simMax) {
                        simMax = simAvg;
                        maxSimCombineWord = resultWord;
                    }
                }
                System.out.println("全局相似度最大的组合词" + maxSimCombineWord + " 相似度为：" + simMax);
                resultSet.add(maxSimCombineWord + "-" + result);
            }
        }
